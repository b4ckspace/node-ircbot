...
class Karma {
    ...
    onKarma(channel, nickname, command, payload) {
        let karma = this.karma.getKarma(nickname);
    ...
    onMessage(channel, nickname, message, isQuery) {
        ...
        const matches = message.match(REGEX_KARMA_INCREMENT);
        if (matches) {
            const favoredNickname = matches[1];
            ...
            let karma = this.karma.incrementKarma(favoredNickname);
