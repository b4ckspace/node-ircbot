...
Karma.prototype.onMessage = function(from, channel, message, isQuery) {

    if(isQuery) {
        // TODO: Fix answer in query!
        //return this.irc.answer(from, channel, 'You can only give karma in the channel');
    }
    ...
    if(matches) {
        ...
        if(commons.sanitizeNickname(from) == commons.sanitizeNickname(nickname)) {
            return this.irc.answer(from, channel, 'You can\'t give yourself karma!');
        }

        this.irc.answer(from, channel, nickname + ' has now xx karma!');
    }
};
